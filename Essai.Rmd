...
@Pallara_Renne_2023 and @Renne_Pallara_2023 investigate the pricing the sovereign credit risk in the context of shadow-intensity frameworks. In such framework, the pricing of defaultable bonds requires the computation of specific conditional expectations of the form:
\begin{equation}
\mathbb{E}_t(\exp[\color{blue}{-z_{t+1}-\dots-z_{t+h}}-\max(0,\lambda_{t+1})-\dots -\max(0,\lambda_{t+h})]),(\#eq:condExp)
\end{equation}
where both $z_t$ and $\lambda_t$ are affine functions of the state vector $x_t$, and where $x_t$ follows a Gaussian vector auto-regressive (VAR) model:
\begin{eqnarray*}
z_t &=& \dot{a}'x_t + \dot{b}\\
\lambda_t &=& a'x_t + b
\end{eqnarray*}
...
Absent the blue terms, the conditional expectations appearing in \@ref(eq:condExp) would be of the same type as the ones considered in the shadow-rate literature (see @Black_1995). In that case, one could for instance used the approaches proposed by @Priebsch_2013 or @Wu_Xia_2016, to calculate approximations for these conditional expectations. @Pallara_Renne_2023 and @Renne_Pallara_2023 extend the approach of @Wu_Xia_2016 to compute approximate values to \@ref(eq:condExp). This page uses the latter approach, employing codes contained in package `TSModels` to implement this approach.
...
Consider a two-factor model, that is $x_t = [x_{1,t},x_{2,t}]'$. Assume that $z_t$ (that can be interpreted as the risk-free short term rate) depends on $x_{1,t}$ only. And $\lambda_t$ (the shadow default intensity) depends on $x_{2,t}$.
...
```{r chartspreads, fig.cap="Term structures of risk-free yields (in red), and credit-risky yields (in black). Solid lines (respectively dashed lines) correspond to the date where $\\lambda_t$ is the highest (resp. the lowest)."}
plot(y[t.max.lambda,],type="l",ylim=c(0,max(y)),xlab="maturity",ylab="yields",lwd=2)

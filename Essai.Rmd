...
\end{equation}
where $z_t$ (risk-free short-term rate) and $\lambda_t$ (*shadow default intensity*) are affine functions of the state vector $x_t$, and where $x_t$ follows a Gaussian vector auto-regressive (VAR) model:
\begin{eqnarray*}
z_t &=& \dot{a}'x_t + \dot{b}\\
\lambda_t &=& a'x_t + b,
\end{eqnarray*}
